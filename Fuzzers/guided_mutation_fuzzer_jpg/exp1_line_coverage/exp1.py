...
def run_mutation_coverage_fuzzer_line_coverage_jpg(run_time_in_min, dir_path_git, seed_path):
    timeout = 60*run_time_in_min
    delete_coverage_files(dir_path=dir_path_git)
    exe_path = fuzz_compile(dir_path=dir_path_git, exec_name="guided_line_mutation_fuzzer")
    ...
    while time.time() < timeout_start + timeout:
        result, outcome, mutated_seed, coverage = mutation_guided_fuzzer.run_branch_coverage(runner=runner_fuzzing)
